...
var flush_stats = function graphite_flush(ts, metrics) {
  ...
  for (key in timers) {
    if (timers[key].length > 0) {
      ...
      message += 'timer.' + key + '.std ' + stddev  + ' ' + ts + "\n";
      message += 'timer.' + key + '.upper ' + max   + ' ' + ts + "\n";
      message += 'timer.' + key + '.lower ' + min   + ' ' + ts + "\n";
      message += 'timer.' + key + '.count ' + count + ' ' + ts + "\n";
      message += 'timer.' + key + '.sum ' + sum  + ' ' + ts + "\n";
      message += 'timer.' + key + '.mean ' + mean + ' ' + ts + "\n";
      statString += message;
  ...
  for (key in gauges) {
    statString += 'gauge.' + key + ' ' + gauges[key] + ' ' + ts + "\n";
    numStats += 1;
  }

  for (key in sets) {
    statString += 'set.' + key + '.count ' + sets[key].values().length + ' ' + ts + "\n";
    numStats += 1;
  }
